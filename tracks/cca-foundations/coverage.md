...
**Общий прогресс: ~14%** (обновлено 2026-09-17 вечер, Europe/Chisinau)

> Оценка консервативная: три темы (D1 + D2 + D3 intro) + 15 practice Q. Exam readiness по домену ниже доли строк карты.
...
| **Итого** | **100%** | **30** | **8** | **5** | **17** | — | — | **~14%** |

\*D5 ~2%: только кросс-касание durable/case-facts из темы D1; отдельной темы D5 не было.

## Что открыл drop 2026-09-17 evening (D3)

- Иерархия CLAUDE.md: managed / user / project / local; **concat** (не silent full override); lazy nested
- `@imports`, AGENTS.md через import, цель &lt;200 строк
- `.claude/rules` + `paths` globs (условная загрузка)
- Slash-команды vs Skills (JIT процедуры)
- Plan Mode vs default/acceptEdits (light)
- 5 practice Q (team project file, globs, Skills, concat, Plan Mode)
- **Не покрыто:** CI/CD `-p` (3.6), iterative refinement (3.5) deep
...
- D3: CI/CD `-p` / structured output (3.6); iterative refinement (3.5); Plan Mode deeper approval flow; permissions layers
- D4 полностью; D5 почти полностью
- Hands-on Messages API tool-loop / Claude Code repo workflow

Practice notes: [`practice/README.md`](practice/README.md) — покрыто **15** (три сессии).
